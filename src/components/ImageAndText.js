...
const ImageAndText = ({ data }) => {
  ...
  const showIcon = (text) => {
    switch (text) {
      ...
      case "Clouds":
        return data.country === "IN" && time.getHours() >= 18 ? (
          <img src={cloudyIconNight} alt="nightIcon" className="h-32 w-32" />
        ) : (
          <img src={cloudyIcon} alt="cloud" className="h-32 w-32 mb-0" />
        );
      ...
      case "Drizzle":
        return data.country === "IN" && time.getHours() >= 18 ? (
          ...
          <img src={drizzleCloudIcon} alt="drizzle" className="h-40 w-40" />
        );
      case "Fog":
      ...
      case "Tornado":
        return (
          <img src={tornadoCloudIcon} alt="tornado" className="h-32 w-32" />
        );
      default:
  ...
  return (
    <div className="mt-5">
      ...
      <div className="flex justify-center text-5xl mt-5">
        <label>{data.text}</label>
      </div>
      <div>
        <div className="mt-5 text-6xl flex justify-center">
          {data.temperature}°C
        </div>
        <label className="flex justify-center mt-2">
          feels like {data.ft}°C
        </label>
        <div className="flex justify-between border-b pb-3 lg:w-4/5 sm:w-2/5 md:w-4/5 m-auto mt-5">
          <label>City</label>
          <span>{data.city}</span>
        </div>
        <div className="flex justify-between border-b pb-3 lg:w-4/5 sm:w-2/5 md:w-4/5 m-auto mt-5">
        ...
        <div className="flex justify-between lg:w-4/5 sm:w-2/5 md:w-4/5 m-auto mt-5">
